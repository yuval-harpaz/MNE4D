...
epochs = mne.Epochs(raw, events, event_id, tmin=-0.1, tmax=0.3, baseline=(None, 0), preload=True)
...
src = mne.setup_source_space(subject, subjects_dir=subjects_dir, add_dist=False, overwrite=True)
...
fwd = mne.make_forward_solution(raw.info, trans, src, bem, fname='inbal4temp-fwd.fif', meg=True, eeg=False, mindist=5.0, n_jobs=2, overwrite=True)
...
brain = surfer.Brain(subject, 'both', 'white', views='caudal',subjects_dir=subjects_dir)
brain.add_data(stcL, colormap=colormap, vertices=verticesL, smoothing_steps=10,  hemi='lh')
brain.add_data(stcR, colormap=colormap, vertices=verticesR, smoothing_steps=10, hemi='rh')
maxval=max(stcL.max(),stcR.max())
brain.scale_data_colormap(fmin=0, fmid=maxval/2, fmax=maxval, transparent=True)
...
bemOrig=subjects_dir+'/'+subject+'/bem/'+subject+'-5120-bem-sol.fif'

# Note that forward solutions can also be read with read_forward_solution
fwdOrig = mne.make_forward_solution(raw.info, transOrig, srcOrig, bemOrig, meg=True, eeg=False, mindist=5.0, n_jobs=2, overwrite=True)
...
colormap = 'hot'
# export SUBJECTS_DIR=/usr/local/freesurfer/subjects/
brainO = surfer.Brain(subject, 'both', 'white', views='caudal',subjects_dir=subjects_dir)
brainO.add_data(stcL, colormap=colormap, vertices=verticesL, smoothing_steps=10,  hemi='lh')
brainO.add_data(stcR, colormap=colormap, vertices=verticesR, smoothing_steps=10, hemi='rh')
maxval=max(stcL.max(),stcR.max())
brainO.scale_data_colormap(fmin=0, fmid=maxval/2, fmax=maxval, transparent=True)

stcOrig.plot(subject=subject,surface='pial',hemi='both',colormap='hot',smoothing_steps=10,transparent=True,subjects_dir=subjects_dir,time_viewer=True)